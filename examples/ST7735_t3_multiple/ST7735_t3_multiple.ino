...
SSD1351_t3 tft = SSD1351_t3(TFT_CS, TFT_DC, TFT_RST);

#if SPI_INTERFACES_COUNT > 1
// Again only one
SSD1351_t3 tft1 = SSD1351_t3(TFT_CS1, TFT_DC1, TFT_MOSI1, TFT_SCLK1, TFT_RST1);

#if SPI_INTERFACES_COUNT > 2
SSD1351_t3 tft2 = SSD1351_t3(TFT_CS2, TFT_DC2, TFT_MOSI2, TFT_SCLK2, TFT_RST2);
#endif
#endif
...
void loop()
{
  ...
  for (uint8_t i = 0; i < sizeof(tfts) / sizeof(tfts[0]); i++)
  {
    if ((millis() > next_test_start_time[i]) && !tfts[i] -> asyncUpdateActive())
    {
      ...
      {
        case 0:
          testlines(tfts[i], SSD1351_YELLOW);
          break;
        case 1:
          testfastlines(tfts[i], SSD1351_RED, SSD1351_BLUE);
          break;
        case 2:
            testdrawrects(tfts[i], SSD1351_GREEN);
            break;
        case 3:
            testfillrects(tfts[i], SSD1351_YELLOW, SSD1351_MAGENTA);
            break;
        case 4:
            testcircles(tfts[i], 10, SSD1351_BLUE);
            break;
        case 5:
...
// Test functions
//=============================================================================
void testlines(SSD1351_t3 *_ptft, uint16_t color) {
  _ptft->fillScreen(SSD1351_BLACK);
...
void testdrawtext(SSD1351_t3 *_ptft, const char *text, uint16_t color) {
  _ptft->setCursor(0, 0);
...
void testfastlines(SSD1351_t3 *_ptft, uint16_t color1, uint16_t color2) {
  _ptft->fillScreen(SSD1351_BLACK);
...
void testdrawrects(SSD1351_t3 *_ptft, uint16_t color) {
  _ptft->fillScreen(SSD1351_BLACK);
  for (int16_t x = 0; x < _ptft->width(); x += 6) {
...
void testfillrects(SSD1351_t3 *_ptft, uint16_t color1, uint16_t color2) {
  _ptft->fillScreen(SSD1351_BLACK);
...
void testcircles(SSD1351_t3 *_ptft, uint8_t radius, uint16_t color) {
  _ptft->fillScreen(SSD1351_BLACK);
...
void testtriangles(SSD1351_t3 *_ptft) {
  _ptft->fillScreen(SSD1351_BLACK);
  int color = 0xF800;
  int t;
  int w = _ptft->width() / 2;
...
void testroundrects(SSD1351_t3 *_ptft) {
  _ptft->fillScreen(SSD1351_BLACK);
  int color = 100;
  int i;
  int t;
...
void tftPrintTest(SSD1351_t3 *_ptft) {
  _ptft->setTextWrap(false);
  _ptft->fillScreen(SSD1351_BLACK);
  _ptft->setCursor(0, 30);
  _ptft->setTextColor(SSD1351_RED);
  _ptft->setTextSize(1);
  _ptft->println("Hello World!");
  _ptft->setTextColor(SSD1351_YELLOW);
  _ptft->setTextSize(2);
  _ptft->println("Hello World!");
  _ptft->setTextColor(SSD1351_GREEN);
  _ptft->setTextSize(3);
  _ptft->println("Hello World!");
  _ptft->setTextColor(SSD1351_BLUE);
  _ptft->setTextSize(4);
